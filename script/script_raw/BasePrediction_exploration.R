...
yieldData_meteo <- read.csv("./data/data_processed/yieldData_meteo")
...
dim(yieldData_meteo)
anomaly_correction <-yieldData_meteo %>% group_by(comId) %>% summarise(average_yield = mean(siloMaize))
...
yieldData_meteo_anomaly <- merge(yieldData_meteo, anomaly_correction_df, by=c("comId","year"))
# str(yieldData_meteo_anomaly)
# str(yieldData_meteo)

#### Further use standard name of data.frame ####
yieldData_meteo <- yieldData_meteo_anomaly
...
remove(yieldData_meteo_anomaly)
...
for (i in 1:2){
  ...
  ## Start loop ##
    ...
    yieldData_meteo_year <- yieldData_meteo %>% filter(year == listyear[[m]] )
    # dim(yieldData_meteo_year)
    # str(yieldData_meteo_year)
    ...
    predict_year_anomaly  <- predict_year - yieldData_meteo_year$average_yield
    ...
    predictData_train <- cbind(yieldData_meteo_year[,1:2], predict_year)
